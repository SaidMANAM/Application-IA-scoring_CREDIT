...
import shap
import warnings
from Preprocessing import merging_data
...
#     st.write(shap.summary_plot(shap_values, data, feature_names=list(X.columns),
#                                title='Graphe des variables les plus influantes sur la décision du modèle '))
#     st.write(shap.decision_plot(expected_value, shap_values[idb], data[idb], feature_names=list(X.columns),
#                                 ignore_warnings=True, title='Graphe d\'explication de la décision du modèle '))
#     shap.initjs()
#     st.write(shap.force_plot(expected_value, shap_values[idb], feature_names=list(X.columns)))
#     st.write(shap.plots._waterfall.waterfall_legacy(expected_value, shap_values[idb], feature_names=list(X.columns),
#                                                     max_display=20, matplotlib=True))
